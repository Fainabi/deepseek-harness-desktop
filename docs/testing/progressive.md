...
| 03 | 档案管理与校验规则 | 已实现 | 6 条接线（001、002、004、005、011、014），另有 10 条下沉单测。001/002/004 走「档案」面板，005/011/014 走命令层 + 磁盘断言；整批走壳层车道（`disableDownload`）。`profile.tsx` 补 `dsh-profile-*` 选择器与行的 `data-profile-id`/`-name`/`-active`，共享 `modal.tsx` 补 `dsh-modal*` 与 `data-status`；`beforeAll` 加数据目录归属的失败关闭校验（G-D03-9）。 |
...
| 2026-09 | 批次 `03`（档案管理与校验规则）实现：6 条接线（`001`/`002`/`004` 走「档案」面板，`005`/`011`/`014` 按 G-D03-4 走命令层 + 磁盘断言），整批走**壳层车道**（`disableDownload: true`，不触达 dsh 服务与 iframe）；`src/ui/config/profile.tsx` 补 `dsh-profile-*` 选择器并给行加 `data-profile-id`/`-name`/`-active`，共享 `src/components/modal.tsx` 补 `dsh-modal`/`-cancel`/`-confirm` 与 `data-status`（`Item` 改为透传 `div` 属性）；`beforeAll` 落实 §5.2 数据目录归属的**失败关闭**校验（G-D03-9），每条用例复位 `active_profile` 并删除本批新建的档案；`TC-DSK-L3-03-014` 按 G-D03-6 借安装产物 + 命令层间接断言解析目录 |
